    ...
    char* volName, int volType )
{
    ...
    // truncate volName in place, leave room for NUL
    if (strlen(volName) > MAXNAMELEN) {
        volName[MAXNAMELEN] = '\0';
    }
    //
    // compute and clamp the length in size_t
    size_t rawLen = strlen(volName);
    size_t maxChars = sizeof(root.diskName) - 1;  // typically MAXNAMELEN
    size_t copyLen = rawLen > maxChars ? maxChars : rawLen;
    //
    // explicit cast silences C4267
    root.nameLen = (char)copyLen;
    //
    // copy the bytes and NUL-terminate safely
    memcpy(root.diskName, volName, copyLen);
    root.diskName[copyLen] = '\0';
    //
