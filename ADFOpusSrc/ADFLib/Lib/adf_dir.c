...
// Chiron 2025
// Replaces code further down this file. 
//
 ...
 *	\param	vol     - a pointer to the parent volume structure.
 *	\param	pSect   - the location of the current directory.
 *	\param	oldName - the current entry name.
 *	\param	nPSect  - the location of the new directory.
 *	\param	newName - the new entry name.
 *	\return	RC_OK or RC_ERROR.
 *
 *	Changes the name of the entry oldName located in the dir at pSect to newName, in the directory at npSect.
 */
	...
	SECTNUM nPSect, char *newName)
{
    ...
    char name2[MAXNAMELEN + 1] = { 0 };
    char name3[MAXNAMELEN + 1] = { 0 };
    ...
    //
    /* how many chars can we copy, leaving room for '\0'? */
    size_t maxCopy = sizeof(entry.name) - 1;      // MAXNAMELEN
    size_t rawLen = strlen(newName);
    size_t copyLen = (rawLen > maxCopy ? maxCopy : rawLen);

    /* store length in the 8-bit field */
    entry.nameLen = (char)copyLen;

    /* copy and terminate safely */
    memcpy(entry.name, newName, copyLen);
 ...
 *	\param	vol	  - A pointer to the volume structure.
 *	\param	pSect - SECTNUM at which the entry resides.
 ...
 * adfSetEntryComment
 */
/*!	\brief	Set an entry's comment.
 *	\param	vol		- A pointer to the volume structure.
 *	\param	parSect	- SECTNUM at which the entry resides.
 *	\param	name	- The entry's name.
 *	\param	newCmt  - The new comment string.
 *	\return	RC_OK or RC_ERROR.
 */
    ...
    char* newCmt)
{
    ...
    size_t rawLen = strlen(newCmt);
    size_t maxLen = MAXCMMTLEN;                   // maximum comment length
    size_t copyLen = rawLen > maxLen ? maxLen : rawLen;

    // explicitly cast to char after clamping
    entry.commLen = (char)copyLen;

    // copy the text and NUL-terminate
    memcpy(entry.comment, newCmt, copyLen);
    entry.comment[copyLen] = '\0';
 ...
 *	adfSetEntryAccess
 */
/*!	\brief	Change an entry's property (access) flags.
 *	\param	vol     - a pointer to the current volume structure.
 *	\param	parSect - the SECTNUM at which the entry is located.
 *	\param	name    - the entry's name.
 *	\param	newAcc  - the new access flags.
 *	\return	RC_OK or RC_ERROR.
 */
 ...
 * adfFreeDirList
 */
/*!	\brief	Frees a linked list or a tree of directory entries.
 *	\param	list - The list.
 *	\return	Void.
 */
 ...
 * adfGetRDirEnt
 */
/*	\brief	Returns a linked list which contains the entries of one directory.
 *	\param	vol		- A pointer to the volume structure.
 *	\param	nSect	- SECTNUM at which the entry resides.
 *	\param	recurs	- TRUE to recurse into sub-directories, FALSE if not.
 *	\return	The list, NULL in case of error.
 */
 ...
 *	\param	vol   - 
 *	\param	nSect - 
 *	\return	Returns a linked list which contains the entries of one directory, NULL in case of error..
 *
 *	Equivalent to struct List* adfGetRDirEnt(struct Volume* vol, SECTNUM dir, FALSE).
 */
 ...
 * adfFreeEntry
 */
/*!	\brief	Free a file entry's memory.
 ...
 * adfToRootDir
 */
/*!	\brief	Move the the root directory of a volume.
 *	\param	vol - a pointer to the volume structure.
 *	\return	RC_OK.
 */
 ...
 * adfChangeDir
 */
/*!	\brief	Change the current working directory within a volume.
 *	\param	vol  - a pointer to the volume structure.
 *  \param	name - a string containing the directory name.
 *	\return	RC_OK or RC_ERROR.
 */
 ...
 * adfParentDir
 */
/*!	\brief	Move to the parent of the current directory within a volume.
 *	\param	vol - a pointer to the volume structure.
 *	\return	RC_OK or RC_ERROR.
 */
 ...
 *	\return	Void.
 *
 *	Do no use this function (not an adf one), but you can use its code to learn how to display a
 *	directory entry (in adf_dir.c). Any function using console output like this should only be
 ...
 * adfCreateDir
 */
/*!	\brief	Create a new directory in the specified parent directory.
 *	\param	vol     - the parent volume.
 *	\param	nParent - the new directory's parent sector.
...
RETCODE adfCreateDir(struct Volume* vol, SECTNUM nParent, char* name)
{
    ...
    //
    dir.nameLen = (char)copyLen;                 // explicit cast silences C4267
    memcpy(dir.dirName, name, copyLen);
    dir.dirName[copyLen] = '\0';                 // never overrun the buffer
    ...
    struct bFileHeaderBlock *fhdr)
{
    ...
    size_t copyLen = (rawLen > maxChars ? maxChars : rawLen);
    //
    fhdr->nameLen = (char)copyLen;                   // explicit cast
    memcpy(fhdr->fileName, name, copyLen);
    fhdr->fileName[copyLen] = '\0';
