...
HWND CreateToolBar(HWND hwndParent)
{
	// 1) Init common-controls (toolbars, etc.)
	...
	// 3) Tell the toolbar the size of our TBBUTTON struct
	SendMessage(hwndTB, TB_BUTTONSTRUCTSIZE, (WPARAM)sizeof(TBBUTTON), 0);

	// 4) Create a true-color image list (16×16 icons + mask)
	...
	// 5) Load your 24/32-bit BMP resource and add to the image list
	HBITMAP hbm = (HBITMAP)LoadImage(
		instance,
		MAKEINTRESOURCE(IDB_TBBUTTONS_TRUECOLOR),
		IMAGE_BITMAP,
		0, 0,
		LR_CREATEDIBSECTION
	);
	if (hbm)
	{
		ImageList_AddMasked(himl, hbm, RGB(255, 0, 255));  // magenta mask
		DeleteObject(hbm);
	}

	// 6) Attach the image list to the toolbar
	SendMessage(hwndTB, TB_SETIMAGELIST, 0, (LPARAM)himl);

	// 7) Define and add your buttons
	TBBUTTON tbb[25] = { 0 };

	tbb[0].iBitmap = 0;  tbb[0].idCommand = ID_FIL_NEW;                  tbb[0].fsState = TBSTATE_ENABLED; tbb[0].fsStyle = TBSTYLE_BUTTON;
	tbb[1].iBitmap = 1;  tbb[1].idCommand = ID_FIL_OPEN;                 tbb[1].fsState = TBSTATE_ENABLED; tbb[1].fsStyle = TBSTYLE_BUTTON;
	tbb[2].iBitmap = 2;  tbb[2].idCommand = ID_FIL_CLOSE;                tbb[2].fsState = TBSTATE_ENABLED; tbb[2].fsStyle = TBSTYLE_BUTTON;
	tbb[3].iBitmap = 0;  tbb[3].idCommand = 0;                           tbb[3].fsState = 0;             tbb[3].fsStyle = TBSTYLE_SEP;
	tbb[4].iBitmap = 3;  tbb[4].idCommand = ID_ACTION_RENAME;            tbb[4].fsState = TBSTATE_ENABLED; tbb[4].fsStyle = TBSTYLE_BUTTON;
	tbb[5].iBitmap = 4;  tbb[5].idCommand = ID_ACTION_DELETE;            tbb[5].fsState = TBSTATE_ENABLED; tbb[5].fsStyle = TBSTYLE_BUTTON;
	tbb[6].iBitmap = 5;  tbb[6].idCommand = ID_ACTION_NEWDIRECTORY;      tbb[6].fsState = TBSTATE_ENABLED; tbb[6].fsStyle = TBSTYLE_BUTTON;
	tbb[7].iBitmap = 6;  tbb[7].idCommand = ID_ACTION_UPONELEVEL;        tbb[7].fsState = TBSTATE_ENABLED; tbb[7].fsStyle = TBSTYLE_BUTTON;
	tbb[8].iBitmap = 0;  tbb[8].idCommand = 0;                           tbb[8].fsState = 0;             tbb[8].fsStyle = TBSTYLE_SEP;
	tbb[9].iBitmap = 7;  tbb[9].idCommand = ID_VIEW_SHOWUNDELETABLEFILES; tbb[9].fsState = TBSTATE_ENABLED; tbb[9].fsStyle = TBSTYLE_BUTTON;
	tbb[10].iBitmap = 8;  tbb[10].idCommand = ID_ACTION_UNDELETE;         tbb[10].fsState = TBSTATE_ENABLED; tbb[10].fsStyle = TBSTYLE_BUTTON;
	tbb[11].iBitmap = 0;  tbb[11].idCommand = 0;                          tbb[11].fsState = 0;            tbb[11].fsStyle = TBSTYLE_SEP;
	tbb[12].iBitmap = 9;  tbb[12].idCommand = ID_FIL_INFORMATION;         tbb[12].fsState = TBSTATE_ENABLED; tbb[12].fsStyle = TBSTYLE_BUTTON;
	tbb[13].iBitmap = 0;  tbb[13].idCommand = 0;                          tbb[13].fsState = 0;            tbb[13].fsStyle = TBSTYLE_SEP;
	tbb[14].iBitmap = 10;  tbb[14].idCommand = ID_ACTION_PROPERTIES;      tbb[14].fsState = TBSTATE_ENABLED; tbb[14].fsStyle = TBSTYLE_BUTTON;
	tbb[15].iBitmap = 0;  tbb[15].idCommand = 0;                          tbb[15].fsState = 0;            tbb[15].fsStyle = TBSTYLE_SEP;
	...
	tbb[18].iBitmap = 12;  tbb[18].idCommand = ID_TOOLS_GREASEWEAZLE;   tbb[18].fsState = TBSTATE_ENABLED; tbb[18].fsStyle = TBSTYLE_BUTTON;
	tbb[19].iBitmap = 13;  tbb[19].idCommand = ID_TOOLS_GREASEWEAZLEWRITE;         tbb[19].fsState = TBSTATE_ENABLED; tbb[19].fsStyle = TBSTYLE_BUTTON;
	...
	tbb[23].iBitmap = 0;  tbb[23].idCommand = 0;                         tbb[23].fsState = 0;            tbb[23].fsStyle = TBSTYLE_SEP;
	tbb[24].iBitmap = 16;  tbb[24].idCommand = ID_TOOLS_OPTIONS;         tbb[24].fsState = TBSTATE_ENABLED; tbb[24].fsStyle = TBSTYLE_BUTTON;

	SendMessage(hwndTB, TB_ADDBUTTONS, (WPARAM)25, (LPARAM)&tbb);
	SendMessage(hwndTB, TB_AUTOSIZE, 0, 0);
	ShowWindow(hwndTB, SW_SHOW);
